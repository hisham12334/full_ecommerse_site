...
async function initializeTables(dbPool) {
  ...
  try {
    ...
    const adminResult = await client.query("SELECT 1 FROM users WHERE role = 'admin'");
    if (adminResult.rows.length === 0) {
      const hashedPassword = await bcrypt.hash('Qadrfits@2025', 10);
      await client.query("INSERT INTO users (name, email, password, role) VALUES ($1, $2, $3, $4)", ['Admin', 'qadr.fits@gmail.com', hashedPassword, 'admin']);
